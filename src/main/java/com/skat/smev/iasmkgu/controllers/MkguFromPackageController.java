...
import com.skat.smev.iasmkgu.domain.db.views.SendedPackageInfo;
...
/**
 * Контроллер для выполнения логики работы с БД,
 * формирования и отправки запросов в СМЭВ
...
public class MkguFromPackageController {
    ...
    @Value("${db.scheduling.dateFrom:2018-01-01}")
    ...
    private void prepareRequestAndSend(MkguBasePackageService mkguBasePackageService){
        ...
        while(dateFrom.before(currentDate)){
            ...
            LOGGER.info("Try to find packages from " + dateFrom.toString() + " to " + dateTo.toString());
            ...
            if(allMkguIds == null || allMkguIds.isEmpty()){
                LOGGER.info("Could not find packages for period: " + dateFrom.toString());
            ...
            for (String mkguId : allMkguIds) {
                ...
                try {
                    LOGGER.info("Prepare rates started: mkru_id =" + mkguId + "; from = " + dateFrom.toString() + "; to = " + dateTo.toString());
                ...
                try {
                    LOGGER.info("Send rate request started: mkru_id =" + mkguId + "; from = " + dateFrom.toString() + "; to = " + dateTo.toString());
                ...
                LOGGER.info("Processing sendedPackageInfoList started. Items count: " + sendedPackageInfoList.size());
                for (SendedPackageInfo sendedPackageInfo : sendedPackageInfoList) {
                    try {
                        int keyId = sendedPackageInfo.getId_key();
                        int otdelId = sendedPackageInfo.getId_otdel();
                        LOGGER.info("Update History By Id Key And Id Otdel started : mkru_id =" + mkguId + "; key = " + keyId + "; oldel = " + otdelId);
                        qualHistoryService.updateHistoryByIdKeyAndIdOtdel(keyId, otdelId);
                    } catch (Exception e) {
                        LOGGER.error("Update HistoryByIdKeyAndIdOtdel failed mkgu_id = " + mkguId + "; error: " + e.getMessage());
                        continue;
                    }
                }
            }
    ...
    private RatesRequest.Forms.Form prepareMkguForm(MkguFormInfo mkguFormInfo, RatesObjectFactory factory) throws DatatypeConfigurationException {
